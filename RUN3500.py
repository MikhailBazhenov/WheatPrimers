...
for item in f3:
    ...
    print('Making primers for 2500-3000 bp PCR products')
    result = subprocess.run(
        ['python3', 'taskmaker.py', 'target_sequence_masked.txt', '50', '2500-3000', 'offtarget_sequences.txt'],
        capture_output=True, encoding='utf-8')
    out = result.stdout
    result = subprocess.run(['primer3_core'], input=out, capture_output=True, encoding='utf-8')
    f1 = open('primer3_out.txt', 'w')
    ...
    result = subprocess.run(['python3', 'out-format.py', 'primer3_out.txt'], capture_output=True, encoding='utf-8')
    f1 = open('primers_2500.txt', 'w')
    f1.write(result.stdout)
    f1.close()
    
    print('Making primers for 3000-3500 bp PCR products')
    result = subprocess.run(
        ['python3', 'taskmaker.py', 'target_sequence_masked.txt', '50', '3000-3500', 'offtarget_sequences.txt'],
        capture_output=True, encoding='utf-8')
    out = result.stdout
    result = subprocess.run(['primer3_core'], input=out, capture_output=True, encoding='utf-8')
    f1 = open('primer3_out.txt', 'w')
    ...
    result = subprocess.run(['python3', 'out-format.py', 'primer3_out.txt'], capture_output=True, encoding='utf-8')
    f1 = open('primers_3000.txt', 'w')
    f1.write(result.stdout)
    f1.close()
    
    result = subprocess.run(['cat', 'primers_500.txt', 'primers_1000.txt', 'primers_1500.txt', 'primers_2000.txt',
                             'primers_2500.txt', 'primers_3000.txt'], capture_output=True, encoding='utf-8')
